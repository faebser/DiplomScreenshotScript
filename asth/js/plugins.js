...
//overview
(function($) {
	var oneImage = function(jQueryObject) {
		var returnImage = {
				me : jQueryObject,
				overlay : jQueryObject.parent().find('.overlay'),
				orgHeight: jQueryObject.data('height'),
				...
				setIcons: function() {
					this.fullscreen.css({ left: this.me.outerWidth()/2 - this.fullscreen.outerWidth(), top: this.overlay.outerHeight()/2 - 16 });
					this.addTo.css({ left: this.me.outerWidth()/2 + parseInt(this.addTo.css('paddingLeft')), top: this.overlay.outerHeight()/2 - 16 });
					this.fuHint.css({ paddingLeft: (this.me.outerWidth() - this.fuHint.outerWidth()) /2, paddingTop: '10px'});
					this.aTHint.css({ paddingLeft: (this.me.outerWidth() - this.aTHint.outerWidth()) /2, paddingTop: '10px'});
				}
		};
		return returnImage;
	};
	var methods = {
			init: function() {
				left = {
						me : $('#leftPicLink'),
						image : oneImage($('#leftPicLink #leftPic'))
				};
				topWrapper = {
						me : $('#topPicsWrapper'),
						gif : oneImage($('#topPicsWrapper #gif')),
			...
			bindEvents: function() {
				$('.icon.fullscreen').click(function(e) {
					e.preventDefault();
					//display overlay and bigsize image
	...
	$.images = function(method) {
		...
		if ( methods[method] ) {
			...
			$.error( 'Method ' +  method + ' does not exist on jQuery.faebser.images' );
		}
	};
})(jQuery);
function resize() {
	if($(window).width()-70 > content.outerWidth(true) + left.image.orgWidth) { // big window
		...
		wrapper.css('width', $(window).width()-70);
		left.image.me.css('width', wrapper.outerWidth() - content.outerWidth() - parseInt(left.image.me.css('marginLeft'))*2);
		topWrapper.gif.me.css('width', left.image.me.css('width'));
		topWrapper.slices.me.css('height', parseInt(topWrapper.gif.me.css('height')));
		topWrapper.me.css('width', topWrapper.gif.me.outerWidth(true) + topWrapper.slices.me.outerWidth(true));
	}
	left.image.setOverlay();
	left.image.setIcons();
	topWrapper.gif.setOverlay();
	topWrapper.gif.setIcons();
	topWrapper.slices.setOverlay();
	topWrapper.slices.setIcons();
}